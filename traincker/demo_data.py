"""Client factice pour le mode démo : données plausibles sans clé API réelle."""

import random
from datetime import datetime, timedelta

_GARES_DEMO = [
    {"id": "demo:1", "name": "Paris Gare de Lyon"}, {"id": "demo:2", "name": "Lyon Part-Dieu"},
    {"id": "demo:3", "name": "Marseille Saint-Charles"}, {"id": "demo:4", "name": "Bordeaux Saint-Jean"},
    {"id": "demo:5", "name": "Lille Flandres"}, {"id": "demo:6", "name": "Strasbourg"},
    {"id": "demo:7", "name": "Nantes"}, {"id": "demo:8", "name": "Toulouse Matabiau"},
]
...
class DemoNavitiaClient:
    ...
    def get_next_departures(self, stop_area_id: str, count: int = 10) -> list[dict]:
        ...
        for i in range(count):
            ...
            heure_prevue = heure_theorique + (timedelta(minutes=random.randint(3, 18)) if en_retard else timedelta())
            departs.append({
...
def obtenir_favoris_demo():
    ...
    return [
        Trajet(nom="Domicile -> Travail", gare_depart_id="demo:1", gare_depart_nom="Paris Gare de Lyon",
               gare_arrivee_id="demo:2", gare_arrivee_nom="Lyon Part-Dieu", actif=True),
        Trajet(nom="Week-end famille", gare_depart_id="demo:2", gare_depart_nom="Lyon Part-Dieu",
               gare_arrivee_id="demo:3", gare_arrivee_nom="Marseille Saint-Charles", actif=True),
    ]
